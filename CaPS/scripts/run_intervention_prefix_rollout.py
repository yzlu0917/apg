...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prefix-based intervention rollouts.")
    parser.add_argument(
        "--intervention-file",
        default="artifacts/object_gate/interventions/micro_batch_v0.jsonl",
        help="Intervention micro-batch JSONL to execute.",
    )
    parser.add_argument(
        "--mode",
        choices=["smoke", "all"],
        default="smoke",
        help="smoke runs one high-dependency and one shallow source; all runs the full ready-only micro-batch.",
    )
    parser.add_argument(
        "--output",
        default="",
...
def choose_records(mode: str, intervention_file: str) -> list[dict]:
    rows = [json.loads(line) for line in (ROOT / intervention_file).open("r", encoding="utf-8")]

    if mode == "all":
        return sorted(
            rows,
            key=lambda r: (
                r["source_rollout_file"],
                r["prompt_id"],
                r["step_index"],
                {"delete": 0, "paraphrase": 1, "distractor": 2}[r["variant_type"]],
            ),
        )
...
def prefix_lines_for_record(record: dict) -> list[str]:
    lines = record["reasoning_lines"]
    if record["variant_type"] == "delete":
        return lines[: record["step_index"]]
    return lines[: record["step_index"] + 1]


def build_user_prompt(raw_question: str, prefix_lines: list[str]) -> str:
    prefix_block = "\n".join(prefix_lines)
    return (
        "You are solving a verifiable reasoning task.\n"
        "The reasoning prefix below is fixed context. Continue from it without repeating earlier lines.\n"
        "Add at most 2 short reasoning lines, then close <reasoning> and provide the final answer inside <final> tags.\n"
...
def run_api(record: dict, manifest_row: dict, api_cfg: dict) -> tuple[str, dict]:
    prefix_lines = prefix_lines_for_record(record)
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Continue only from the given reasoning prefix. "
                    "Do not restate the existing plan. "
                    "Add at most 2 short reasoning lines, then return <final>."
                ),
            },
            {"role": "user", "content": build_user_prompt(manifest_row["raw_question"], prefix_lines)},
        ],
        "temperature": 0.0,
        "max_tokens": 220,
    }
...
def run_local(records_with_manifest: list[tuple[dict, dict]]) -> list[tuple[str, dict]]:
    ...
    for record, manifest_row in records_with_manifest:
        ...
        user_prompt = (
            "You are solving a verifiable reasoning task.\n"
            "Continue the reasoning from the fixed prefix and then provide the final answer.\n"
            "Do not repeat earlier reasoning lines.\n"
            f"Problem:\n{manifest_row['raw_question']}\n"
        )
        assistant_prefill = "<reasoning>\n"
        if prefix_block:
            assistant_prefill += prefix_block + "\n"
        messages = [
...
def main() -> None:
    args = parse_args()
    records = choose_records(args.mode, args.intervention_file)
    manifest_map = load_manifest_map()
    api_cfg = load_api_config()
    if args.output:
        output_path = ROOT / args.output
    elif args.mode == "all":
        output_path = ROOT / "artifacts" / "object_gate" / "analysis" / "intervention_prefix_rollout_batch_000.jsonl"
    else:
        output_path = ROOT / "artifacts" / "object_gate" / "analysis" / "intervention_prefix_rollout_smoke_000.jsonl"
                ...
                json.dumps(
                    {
                        ...
                        "prefix_line_count": len(prefix_lines_for_record(record)),
                        "raw_completion": text,
                        "final": final,
