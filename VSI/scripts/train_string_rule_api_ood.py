#!/usr/bin/env python3
"""Evaluate coverage-vs-shift on a non-numeric API-backed exploit family."""
...
def rand_word(rng: random.Random, min_len: int = 4, max_len: int = 7) -> str:
...
def encode_text(text: str, max_len: int = 16) -> torch.Tensor:
...
def train_model(
    train_tasks: Sequence[Dict[str, object]],
    labels: Sequence[str],
) -> Tuple[torch.nn.Module, Dict[str, int], List[float]]:
    ...
    model = torch.nn.Sequential(
        torch.nn.Linear(xs.shape[1], 128),
        torch.nn.ReLU(),
    ...
    return model, label_to_idx, losses
...
def eval_model(model: torch.nn.Module, tasks: Sequence[Dict[str, object]], labels: Sequence[str]) -> Dict[str, float]:
    ...
    for task in tasks:
        ...
        hidden_inputs = [item[0] for item in task["hidden_tests"]]
        pred_outputs = [apply_label(predicted, text) for text in hidden_inputs]
...
def summarize_regime(include_ood: bool, eval_tasks: Sequence[Dict[str, object]]) -> Dict[str, Any]:
    ...
    model, _, losses = train_model(train_tasks, labels)
    return {
        ...
        "api_ood_eval": eval_model(model, eval_tasks, labels),
    }
