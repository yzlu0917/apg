...
def fit_contrastive_embedding_scorer(
    ...
    for _ in range(epochs):
        ...
        if int(same_mask.sum().item()) > 0:
            loss_terms.append(same_weight * torch.mean(distances[same_mask] ** 2))
        if int(flip_mask.sum().item()) > 0:
            loss_terms.append(flip_weight * torch.mean(torch.nn.functional.softplus(margin - distances[flip_mask])))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate CTS with contrastive embedding scorers.")
    ...
    for theorem_id, test_indices in group_to_indices.items():
        ...
        for field, baseline_name in [("h_plus", "post_contrastive"), ("delta_h", "transition_contrastive")]:
            ...
            torch.manual_seed(0)
            src, var = fit_contrastive_embedding_scorer(
