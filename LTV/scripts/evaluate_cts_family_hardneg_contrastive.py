...
from evaluate_cts_hardneg_contrastive import aggregate_metrics, flatten_field
...
def fit_family_hardneg_contrastive_scorer(
    ...
    if len(train_source) == 0:
        zeros_src = torch.zeros(len(test_source), dtype=torch.float32)
        zeros_var = torch.zeros(len(test_variant), dtype=torch.float32)
        return zeros_src, zeros_var
    ...
    flip_mask = ~same_mask
    targeted_flip_mask = torch.tensor(
        [fam == target_flip_family for fam in train_flip_families], dtype=torch.bool
    ) & flip_mask
    ...
    for _ in range(epochs):
        ...
        if int(flip_mask.sum().item()) > 0:
            flip_losses = torch.nn.functional.softplus(margin - distances[flip_mask])
            flip_weights = torch.ones_like(flip_losses)
            if int(targeted_flip_mask.sum().item()) > 0:
                flip_weights[targeted_flip_mask[flip_mask]] = target_weight
            loss_terms.append(torch.mean(flip_weights * flip_losses))
        ...
        if int(same_mask.sum().item()) > 0 and int(flip_mask.sum().item()) > 0:
            ...
            if int(targeted_flip_mask.sum().item()) > 0:
                targeted_neg_pool = var_z[targeted_flip_mask]
                targeted_hard_neg_dist = torch.cdist(same_src, targeted_neg_pool).min(dim=1).values
                targeted_triplet = torch.nn.functional.softplus(pos_dist + margin - targeted_hard_neg_dist)
                loss_terms.append(hardneg_weight * (target_weight - 1.0) * torch.mean(targeted_triplet))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate CTS with family-targeted hard-negative contrastive scorers.")
    ...
    parser.add_argument("--target-flip-family", required=True)
    parser.add_argument("--target-weight", type=float, default=2.0)
    parser.add_argument("--baseline-prefix", default="familyhardneg")
    ...
    for row in cts_rows:
        ...
        meta = annotated_map[row["pair_id"]]
        extracted_pairs.append(
            {
                "pair_id": row["pair_id"],
                "type": row["type"],
                "flip_family": meta.get("flip_family"),
    ...
    for theorem_id, test_indices in group_to_indices.items():
        ...
        train_flip_families = [extracted_pairs[i]["flip_family"] for i in train_indices]
        ...
        for field, baseline_name in [("h_plus", "post_contrastive"), ("delta_h", "transition_contrastive")]:
            ...
            torch.manual_seed(0)
            src, var = fit_family_hardneg_contrastive_scorer(
                train_source=train_source,
                train_variant=train_variant,
                train_types=train_types,
                train_flip_families=train_flip_families,
                test_source=test_source,
                test_variant=test_variant,
                hardneg_weight=args.hardneg_weight,
                target_flip_family=args.target_flip_family,
