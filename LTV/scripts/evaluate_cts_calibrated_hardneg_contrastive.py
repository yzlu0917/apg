...
def fit_calibrated_hardneg_contrastive_scorer(
    ...
    def class_score_terms(src_z: torch.Tensor, var_z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        item_embeds = [src_z]
        item_labels = [torch.ones(len(src_z), dtype=torch.float32, device=src_z.device)]

        same_variant = var_z[same_mask]
        if len(same_variant) > 0:
            item_embeds.append(same_variant)
            item_labels.append(torch.ones(len(same_variant), dtype=torch.float32, device=src_z.device))

        flip_variant = var_z[flip_mask]
        if len(flip_variant) > 0:
            item_embeds.append(flip_variant)
            item_labels.append(torch.zeros(len(flip_variant), dtype=torch.float32, device=src_z.device))
        ...
        pos_dist = torch.norm(item_embeds - pos_centroid, dim=1)
        neg_dist = torch.norm(item_embeds - neg_centroid, dim=1)
        logits = neg_dist - pos_dist
        return logits, item_labels, item_embeds
    ...
    for _ in range(epochs):
        ...
        logits, labels, _ = class_score_terms(src_z, var_z)
        calib_loss = torch.nn.functional.binary_cross_entropy_with_logits(logits, labels)
        loss_terms.append(calibration_weight * calib_loss)
    ...
    with torch.no_grad():
        src_z = encoder(train_source)
        var_z = encoder(train_variant)
        train_logits, train_labels, train_items = class_score_terms(src_z, var_z)
        pos_centroid = train_items[train_labels == 1].mean(dim=0, keepdim=True)
        neg_centroid = train_items[train_labels == 0].mean(dim=0, keepdim=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate CTS with calibration-aware hard-negative contrastive scorers.")
    ...
    parser.add_argument("--baseline-prefix", default="calhardneg")
    ...
    for theorem_id, test_indices in group_to_indices.items():
        ...
        for field, baseline_name in [("h_plus", "post_contrastive"), ("delta_h", "transition_contrastive")]:
            ...
            torch.manual_seed(0)
            src, var = fit_calibrated_hardneg_contrastive_scorer(
