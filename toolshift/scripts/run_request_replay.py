...
from toolshift.request_replay import run_request_replay_sanity
...
def _render_markdown(benchmark_path: str, summary: dict) -> str:
    lines = [
        ...
        f"- execute_render_pass_rate: `{summary['execute_render_pass_rate']:.3f}`",
        f"- negative_block_pass_rate: `{summary['negative_block_pass_rate']:.3f}`",
    ...
    for transform_name, transform_summary in summary["by_transform"].items():
        lines.append(
            ...
            f"{transform_summary['pass_rate']:.3f} | {transform_summary['emit_rate']:.3f} |"
        )
    return "\n".join(lines) + "\n"


def main() -> None:
    parser = argparse.ArgumentParser(description="Run request-level replay sanity checks for a ToolShift benchmark.")
    parser.add_argument("--benchmark", default="data/real_evolution_benchmark.json")
    parser.add_argument("--output-dir", default="artifacts/real_evolution_request_replay")
    ...
    records, summary = run_request_replay_sanity(suite)
    ...
    print(
        "count\tpass_rate\texecute_render\tnegative_block\tpositive_equivalence",
        flush=True,
    )
    print(
        f"{summary['count']}\t"
        f"{summary['pass_rate']:.3f}\t"
        f"{summary['execute_render_pass_rate']:.3f}\t"
        f"{summary['negative_block_pass_rate']:.3f}\t"
