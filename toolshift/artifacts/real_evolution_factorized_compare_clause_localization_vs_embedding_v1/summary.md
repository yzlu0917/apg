# Fixed Panel Method Comparison

- Records: `artifacts/real_evolution_family_holdout_clause_localization_capability_v1/records.json`
- Benchmark: `data/real_evolution_benchmark.json`
- Regime: `family_holdout_cv`
- Methods: `semantic_embedding_capability_gate, semantic_clause_localization_capability_gate`

## Overall Metrics

| method | CAA | CAA+ | NOS | POC | coverage | selective_risk | contract_validity |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| semantic_embedding_capability_gate | 1.000 | 1.000 | 1.000 | 1.000 | 0.986 | 0.000 | 1.000 |
| semantic_clause_localization_capability_gate | 0.875 | 0.820 | 0.864 | 0.820 | 0.986 | 0.127 | 1.000 |

## semantic_clause_localization_capability_gate vs semantic_embedding_capability_gate

- improved_pair_count: `0`
- regressed_pair_count: `18`
- improved_distinct_views: `0`
- regressed_distinct_views: `11`
- strictly_fixed_views: `[]`

### Delta Metrics

- CAA_clean: `-0.083`
- CAA_negative: `-0.136`
- CAA_overall: `-0.125`
- CAA_positive: `-0.180`
- NOS: `-0.136`
- POC: `-0.180`
- contract_validity: `0.000`
- coverage: `0.000`
- negative_coverage: `0.000`
- selective_risk: `0.127`

### Incremental Fixes

- by_transform: `{}`
- by_family_tag: `{}`
- by_primary_tool_id: `{}`
- from_group: `{}`
- from_bucket: `{}`

### Regressions

- by_transform: `{"clean": 6, "negative_parent_scope_change": 1, "negative_source_removed": 2, "positive_version_migration": 9}`
- by_family_tag: `{"drive": 6, "notion": 1, "people": 1, "sheets": 4, "stripe": 6}`
- from_bucket: `{"correct_execute": 15, "correct_non_execute": 3}`

### Representative Fixes

- none

## By Family

### semantic_embedding_capability_gate

- `bitbucket`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `confluence`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `drive`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `jira`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `notion`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `people`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `sheets`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `slack`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `stripe`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`

### semantic_clause_localization_capability_gate

- `bitbucket`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `confluence`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `drive`: `CAA=0.625` `CAA+=0.667` `NOS=1.000` `count=16`
- `jira`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `notion`: `CAA=0.938` `CAA+=1.000` `NOS=0.750` `count=16`
- `people`: `CAA=0.938` `CAA+=0.833` `NOS=1.000` `count=16`
- `sheets`: `CAA=0.750` `CAA+=0.333` `NOS=1.000` `count=16`
- `slack`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `stripe`: `CAA=0.625` `CAA+=0.500` `NOS=0.500` `count=16`
