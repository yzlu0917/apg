# Fixed Panel Method Comparison

- Records: `artifacts/real_evolution_family_holdout_description_pool_capability_v1/records.json`
- Benchmark: `data/real_evolution_benchmark.json`
- Regime: `family_holdout_cv`
- Methods: `semantic_embedding_capability_gate, semantic_description_pool_capability_gate`

## Overall Metrics

| method | CAA | CAA+ | NOS | POC | coverage | selective_risk | contract_validity |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| semantic_embedding_capability_gate | 1.000 | 1.000 | 1.000 | 1.000 | 0.986 | 0.000 | 1.000 |
| semantic_description_pool_capability_gate | 0.889 | 0.880 | 0.545 | 0.880 | 0.986 | 0.113 | 1.000 |

## semantic_description_pool_capability_gate vs semantic_embedding_capability_gate

- improved_pair_count: `0`
- regressed_pair_count: `16`
- improved_distinct_views: `0`
- regressed_distinct_views: `8`
- strictly_fixed_views: `[]`

### Delta Metrics

- CAA_clean: `0.000`
- CAA_negative: `-0.455`
- CAA_overall: `-0.111`
- CAA_positive: `-0.120`
- NOS: `-0.455`
- POC: `-0.120`
- contract_validity: `0.000`
- coverage: `0.000`
- negative_coverage: `0.000`
- selective_risk: `0.113`

### Incremental Fixes

- by_transform: `{}`
- by_family_tag: `{}`
- by_primary_tool_id: `{}`
- from_group: `{}`
- from_bucket: `{}`

### Regressions

- by_transform: `{"negative_account_object_removed": 2, "negative_removed_capability": 2, "negative_search_replacement": 2, "negative_source_removed": 2, "negative_space_key_lookup_split": 2, "positive_version_migration": 6}`
- by_family_tag: `{"bitbucket": 2, "confluence": 2, "notion": 2, "people": 2, "sheets": 4, "stripe": 4}`
- from_bucket: `{"correct_execute": 6, "correct_non_execute": 10}`

### Representative Fixes

- none

## By Family

### semantic_embedding_capability_gate

- `bitbucket`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `confluence`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `drive`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `jira`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `notion`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `people`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `sheets`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `slack`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `stripe`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`

### semantic_description_pool_capability_gate

- `bitbucket`: `CAA=0.875` `CAA+=1.000` `NOS=0.000` `count=16`
- `confluence`: `CAA=0.875` `CAA+=1.000` `NOS=0.000` `count=16`
- `drive`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `jira`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `notion`: `CAA=0.875` `CAA+=1.000` `NOS=0.500` `count=16`
- `people`: `CAA=0.875` `CAA+=0.667` `NOS=1.000` `count=16`
- `sheets`: `CAA=0.750` `CAA+=0.333` `NOS=1.000` `count=16`
- `slack`: `CAA=1.000` `CAA+=1.000` `NOS=1.000` `count=16`
- `stripe`: `CAA=0.750` `CAA+=1.000` `NOS=0.000` `count=16`
