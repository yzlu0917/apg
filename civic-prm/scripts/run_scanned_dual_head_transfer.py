...
from civic_prm.scanned_process import aggregate_scanned_rows, expand_step_scan_records
from civic_prm.splits import build_quartet_split_map
from civic_prm.text_views import build_step_only_text_at_locus, build_view_text


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a step-scan dual-head disentangled verifier and evaluate on an OOD generated slice."
    )
    parser.add_argument(
        "--train-dataset",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/generated/model_generated_full_hybrid_counterfactual_v2_natural_scanned_dual_head.json"),
    )
    parser.add_argument(
        "--feature-cache-dir",
        type=Path,
        default=Path("artifacts/generated/full_hybrid_scanned_dual_head_features_v1"),
    )
...
def _load_or_extract_features(
    ...
    if view_name == "step_scan":
        texts = [build_step_only_text_at_locus(record, record["audited_locus"]) for record in records]
    else:
...
def _selection_metric(process_metrics: dict, total_metrics: dict, utility_metrics: dict) -> float:
    return (
...
def main() -> None:
    ...
    val_scan_rows = expand_step_scan_records(val_rows)
    eval_scan_rows = expand_step_scan_records(eval_records)
    ...
    step_train = _load_or_extract_features(
        train_rows,
        "step_scan",
        tokenizer,
        model,
        args.feature_cache_dir / "train_step_only.pt",
        args.batch_size,
        args.max_length,
    )
    step_val = _load_or_extract_features(
        val_rows,
        "step_scan",
        tokenizer,
        model,
        args.feature_cache_dir / "val_step_only.pt",
        args.batch_size,
        args.max_length,
    )
    step_val_scan = _load_or_extract_features(
        val_scan_rows,
        "step_scan",
        tokenizer,
        model,
        args.feature_cache_dir / "val_step_scan.pt",
        args.batch_size,
        args.max_length,
    )
    step_eval_scan = _load_or_extract_features(
        eval_scan_rows,
        "step_scan",
        tokenizer,
        model,
        args.feature_cache_dir / "eval_step_scan.pt",
        args.batch_size,
        args.max_length,
    )
    ...
    best_payload = None
    search_rows = []
    for lambda_local_pair in args.local_pair_grid:
        torch.manual_seed(args.seed)
        process_head = train_repair_head(
            train_features=step_train,
            train_records=train_rows,
            train_labels=_process_labels(train_rows),
            val_features=step_val,
            val_records=val_rows,
            val_labels=_process_labels(val_rows),
            lambda_local_pair=lambda_local_pair,
            lambda_cond_swap=0.0,
        )
        for aggregation in args.aggregation_grid:
            process_val_rows = aggregate_scanned_rows(
                model=process_head,
                train_features=step_train,
                scan_features=step_val_scan,
                scan_records=val_scan_rows,
                original_records=val_rows,
    ...
    process_eval_rows = aggregate_scanned_rows(
        model=best_payload["process_head"],
        train_features=step_train,
        scan_features=step_eval_scan,
        scan_records=eval_scan_rows,
        original_records=eval_records,
        aggregation=best_payload["aggregation"],
        softmin_beta=args.softmin_beta,
    )
    total_eval_rows = _combine_rows(process_eval_rows, consistency_eval_rows, alpha=best_payload["alpha"])
    ...
    outputs = {
        "config": {
            ...
            "seed": args.seed,
            "local_pair_grid": args.local_pair_grid,
            "alpha_grid": args.alpha_grid,
            "aggregation_grid": args.aggregation_grid,
            "softmin_beta": args.softmin_beta,
        },
        "selected_hparams": {
            "lambda_local_pair": best_payload["lambda_local_pair"],
            "aggregation": best_payload["aggregation"],
